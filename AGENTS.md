...
| `mcp-devtools/vue`    | browser | Vue / Pinia helpers + `mcpDevtoolsVue` plugin: component tree tools (`mcp-devtools_vue_*`)         |
| `mcp-devtools/react`  | browser | React hooks / store helpers + DevTools-style tools (`mcp-devtools_react_*`, hook shim in `<head>`) |
| `mcp-devtools/svelte` | browser | Svelte store / rune helpers (Svelte 5 has no runtime hook for internals, see README)               |
| `mcp-devtools/vite`   | node    | `McpDevtools({ react? })` Vite plugin: bridge + config page + `connect.js` (+ React hook shim)     |
